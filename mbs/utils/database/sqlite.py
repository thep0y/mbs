...
class DataBase:
    ...
    def query_category_for_post(self, title: str) -> Tuple[str, int, int]:
        sql = (
            "SELECT c.category, c.jianshu_id, c.cnblogs_id FROM categories c WHERE c.id = (SELECT p.category_id FROM"
            " posts p WHERE p.title = '%s')" % title
        )
        row = self.execute(sql).fetchone()
        if not row:
    ...
    def select_all_not_uploaded_posts(self):
        sql = (
            "SELECT title, jianshu_id, cnblogs_id, segment_fault_id, file_path  FROM posts WHERE jianshu_id IS NULL OR"
            " cnblogs_id IS NULL OR segment_fault_id IS NULL;"
        )
    ...
    def insert_post(
        ...
        sql = (
            "INSERT INTO `posts` (title, md5, jianshu_id, cnblogs_id, segment_fault_id, category_id, file_path,"
            " create_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?);"
        )
        now = time.localtime()
        create_time = time.strftime("%Y-%m-%d %H:%M:%S", now)
        self.execute(sql, title, md5, jianshu_id, cnblogs_id, sf_id, category_id, file_path, create_time)
        self.commit()
